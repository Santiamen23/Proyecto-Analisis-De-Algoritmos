...
void Grafo::construir(vector<Nodo> nodos, vector<Arista> aristas) {
    ...
    for (int indiceArista = 0; indiceArista < static_cast<int>(this->aristas.size()); ++indiceArista) {
        ...
        const pair<int, int> claveDir = {indiceOrigen, indiceDestino};
        ...
        const pair<int, int> claveDirInversa = {indiceDestino, indiceOrigen};
        ...
        const pair<int, int> claveNoDir = {menor, mayor};
